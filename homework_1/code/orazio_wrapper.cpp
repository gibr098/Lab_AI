...
void OrazioWrapper::connect(const char *serial_device) {
  ...
  if(OrazioClient_sync(_client, 1)!=Success) disconnect();

  //

  // TODO : Try to read the configuration from eeprom,
  //        in the negative case disconnect orazio
  if(OrazioClient_readConfiguration(_client,100)!=Success) disconnect();
  ...
  if(OrazioClient_sendPacket(_client, (PacketHeader *)&system_params,100)!=Success) disconnect();
...
void OrazioWrapper::setVelocities(float tv, float rv) {
  ...
  OrazioClient_sendPacket(_client, (PacketHeader *)&drive_control,100);
...
void OrazioWrapper::getSonar(bool &is_new, float &range, int sonar_num) {
  ...
  //        is_new to true
  range = 0;
  if (sonar_num < 0 || sonar_num >= SONARS_MAX) return;
  
  if(sonar_status.ranges[sonar_num]!=currentEpoch()){
	  is_new=false;
	  }else{
